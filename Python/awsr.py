...
if mode == 'start instance' : 
   ...
   status = gso(f'sudo /usr/local/bin/aws ec2 start-instances --instance-ids {id}')
   print('ec2 instance started!' if status[0]==0 else 'Failed to start!')

elif mode == 'stop instance' :
  
   id = form_values.getvalue('i1')
   status = gso(f'sudo /usr/local/bin/aws ec2 stop-instances --instance-ids {id}')
   ...
   status = gso(f'sudo /usr/local/bin/aws ec2 create-security-group --description {group_des} --group-name {group_name}')
   ...
   status = gso(f'sudo /usr/local/bin/aws s3api create-bucket --bucket {bucket_name} --region {region} --create-bucket-configuration LocationConstraint={region}')
   ...
   status = gso(f'sudo /usr/local/bin/aws s3 cp {path} s3://{bucket_name}/')
   print('file uploaded to bucket!' if status[0]==0 else 'Failed to upload!')

elif mode == 'create cdn' : 
    
   origin = form_values.getvalue('i1')
   status = gso(f'sudo /usr/local/bin/aws cloudfront create-distribution --origin-domain-name {origin}')
